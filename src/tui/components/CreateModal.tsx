...
interface Props {
  onSubmit: (data: { title: string; description: string; column: Column; priority: Priority; tags: string[] }) => void;
  onCancel: () => void;
}

type Field = 'title' | 'description' | 'column' | 'priority' | 'tags';
const FIELDS: Field[] = ['title', 'description', 'column', 'priority', 'tags'];
...
export function CreateModal({ onSubmit, onCancel }: Props) {
  ...
  useInput((input, key) => {
    ...
    if (field === 'column') {
      ...
      if (key.tab || key.downArrow) setField(FIELDS[fieldIdx + 1] ?? 'tags');
  ...
  return (
    <Box flexDirection="column" paddingX={2} paddingY={1} borderStyle="round" borderColor={colors.activeBorder}>
      ...
      <Box marginTop={1}>
        <Text dimColor>Tab/Enter: next field · Esc: cancel · ←→: select option</Text>
      </Box>
    </Box>
  );
}
